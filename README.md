...
<p align="center">
  <a href="https://1hamzaachour-ai.github.io/baristas-coffee-analytics/"><b>🔴 Live Dashboard</b></a> ·
...
<a href="https://1hamzaachour-ai.github.io/baristas-coffee-analytics/">
...
<tr><td>Visualization</td><td>Power BI-style interactive HTML dashboard (<a href="https://1hamzaachour-ai.github.io/baristas-coffee-analytics/">live on GitHub Pages</a>), Power BI Desktop-ready</td></tr>
</table>
...
├── index.html                       # Redirects the Pages root to the dashboard
├── data_architecture/
...
**Just want to see it?** → **[Open the live dashboard](https://1hamzaachour-ai.github.io/baristas-coffee-analytics/)**
— no setup, works in any browser, built from the real generated dataset below.

<details>
<summary><b>Rebuild it yourself</b> (regenerate the data, or recreate the model in actual Power BI Desktop)</summary>
...
1. Generate the dataset:
   ...
   `data_generator/data/`.
2. **Refresh the live dashboard:** `cd dashboard/web && python build_data.py`
   re-encodes those CSVs into `data.js`. Open `index.html` in a browser to
   check it.
3. **Or build it in Power BI Desktop:** for each block in
   `powerbi/power_query_scripts.m`, Get Data → Blank Query → Advanced Editor
   → paste → rename the query to the name in that block's header comment.
   This loads and types all 7 tables in one pass — no manual CSV wizard, no
   table-renaming step. Then: build relationships per `star_schema_design.md`,
   mark `Dim_Date` as the date table, paste in the measures from
   `dax/dax_measures_library.dax`, and recreate the 3 report pages using
   `dashboard/dashboard_wireframe_spec.md`.
